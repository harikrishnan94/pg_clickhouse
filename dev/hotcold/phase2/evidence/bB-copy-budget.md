...
| **[consumer — measured, L0014]** adopted-offsets safety validation | row_count | **scan (not a copy); 14.4%** | `ColumnString::validateAdoptedOffsets()` — a scalar O(rows) monotonicity + terminal-offset check run on every adopted String column, common to ALL adopt transports (bespoke tcp + arrow + shm) | perf: **14.4% of consumer CPU on CB Q24** — a vectorization candidate (general adopt-path win, orthogonal to the arrow-vs-tcp gap), recorded not hidden |
