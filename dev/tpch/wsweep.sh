#!/usr/bin/env bash
# Native-vs-offload W-sweep under a shared cgroup cpu.max cap, for any TPC-H query.
#
# Generalises dev/bench/sweep-capped.sh to run an arbitrary query (extracted from
# dev/tpch/queries/N.sql) and to detect offload eligibility dynamically. Both the
# PostgreSQL postmaster tree AND the co-located ClickHouse server are placed in ONE
# cgroup v2 capped to W cores via cpu.max (quota = W*period), so native and offload
# compete for the SAME core budget -- removing any "offload used more cores"
# confound. The measuring shell stays OUTSIDE the capped cgroup; cgroups are
# restored on exit.
#
...
#
#   QUERIES="1 4 6" W_LIST="2 4 8 16" N=5 dev/tpch/wsweep.sh
set -uo pipefail
...
QUERIES="${QUERIES:-1 4 6}"
RUN_ID="${RUN_ID:-tpchcb}"
QDIR="$HERE/queries"
OUT="${OUT:-$HERE/evidence/phase1/wsweep}"
mkdir -p "$OUT"
...
CG=/sys/fs/cgroup/pgch_wsweep
PERIOD=100000
[ -n "$CHPID" ] && [ -n "$PMPID" ] || { echo "could not resolve CH/PM pids (CH=$CHPID PM=$PMPID)" >&2; exit 1; }
echo "postmaster=$PMPID clickhouse=$CHPID db=$PGDB N=$N W_LIST='$W_LIST' QUERIES='$QUERIES'"
...
new_tag(){ printf 'wsw_%s_%s_%s' "$1" "$$" "${RANDOM}${RANDOM}" | tr -cd 'a-z0-9_'; }
...
SET pg_clickhouse.session_settings='join_use_nulls 1, group_by_use_nulls 1, final 1, allow_experimental_streamed_table_function 1, log_comment $1';
SET pg_clickhouse.enable_shm_offload=on;
SET max_parallel_workers=64; SET max_parallel_workers_per_gather=16;
SET statement_timeout='300s';
SQL
}

# One timed run: prints "wall_s host_cores ch_cores". $1 = SET block, $2 = SQL.
...
# median / min / max / stdev of the wall column (field 1) over stdin lines.
stats(){ awk '{w[NR]=$1; ch[NR]=$3; ho[NR]=$2; s+=$1; ss+=$1*$1}
  END{n=NR; if(n==0){print "0 0 0 0 0 0"; exit}
      ...
      # cores of the median run
      printf "%.4f %.4f %.4f %.4f %.3f %.3f", med, mn, mx, sd, ho[int((n+1)/2)], ch[int((n+1)/2)]}'; }

# Eligibility: returns "blocks read_rows" if the offloaded query is genuinely
# pushed (plan has ClickHouseShmScan AND query_log oracle fires), else "NO".
check_eligible(){ local sql="$1" tag; tag=$(new_tag elig)
  ...
  local m; m=$(chq "SELECT sum(ProfileEvents['ShmAdoptedBlocks']), sum(read_rows) FROM system.query_log WHERE log_comment='$tag' AND type='QueryFinish' AND positionCaseInsensitive(query,'streamed_table')>0 AND positionCaseInsensitive(query,'query_log')=0")
  local blk; blk=$(echo "$m" | cut -f1); blk=${blk:-0}
...
: > "$RESULTS"
{
  echo "# Native-vs-offload W-sweep under shared cgroup cpu.max cap"
  echo ""
  echo "DB=$PGDB  N=$N warm runs/cell  W in {$W_LIST}  postmaster=$PMPID clickhouse=$CHPID"
...
for q in $QUERIES; do
  ...
  # Strip the EXPLAIN wrapper and SQL line-comments, drop trailing semicolons.
  # Keep newlines: flattening to one line would let a "-- ..." comment eat the query.
  ...
  for W in $W_LIST; do
    ...
    per=$((W/2)); [ $per -lt 1 ] && per=1
    # NATIVE: parallel_workers = W per referenced table (best-effort: set on all base tables in query)
    "${PSQLA[@]}" -c "SET search_path=pg; ALTER TABLE lineitem SET (parallel_workers=$W); ALTER TABLE orders SET (parallel_workers=$W); ALTER TABLE customer SET (parallel_workers=$W); ALTER TABLE part SET (parallel_workers=$W);" >/dev/null 2>&1
    printf '%s\n%s;\n' "$(native_set $W)" "$sql" | "${PSQL[@]}" >/dev/null 2>&1   # warm
    nat=$(for _ in $(seq 1 "$N"); do measure "$(native_set $W)" "$sql"; echo; done | stats)
    # OFFLOAD: per-table producers = W/2 (two-table-ish balance), max_threads=16
    "${PSQLA[@]}" -c "SET search_path=pg; ALTER TABLE lineitem SET (parallel_workers=$per); ALTER TABLE orders SET (parallel_workers=$per); ALTER TABLE customer SET (parallel_workers=$per); ALTER TABLE part SET (parallel_workers=$per);" >/dev/null 2>&1
    otag=$(new_tag run)
    printf '%s\n%s;\n' "$(offload_set ${otag}w)" "$sql" | "${PSQL[@]}" >/dev/null 2>&1   # warm
    off=$(for _ in $(seq 1 "$N"); do measure "$(offload_set $otag)" "$sql"; echo; done | stats)
    # a/b fields: 1=med_wall_s 2=min 3=max 4=sd 5=host_cpu_s(median run) 6=ch_cpu_s.
    # Cores = CPU-seconds / wall-seconds of the median run (host-minus-CH = PG side).
  ...
  "${PSQLA[@]}" -c "SET search_path=pg; ALTER TABLE lineitem RESET (parallel_workers); ALTER TABLE orders RESET (parallel_workers); ALTER TABLE customer RESET (parallel_workers); ALTER TABLE part RESET (parallel_workers);" >/dev/null 2>&1
done
echo "" | tee -a "$RESULTS"
